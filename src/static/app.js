  ...
  async function fetchActivities() {
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsListHtml = participants.length > 0
          ? `<div class="participants-list">${participants
              .map((participant) => `
                <span class="participant-item">
                  <span>${participant}</span>
                  <button class="participant-delete-btn" data-activity="${name}" data-email="${participant}" aria-label="Remove ${participant}">✕</button>
                </span>`)
              .join("")}</div>`
